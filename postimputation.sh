...
postimpute(){
  ...
  then
    ...
    zcat $dir/pimputed_files/$prefix/$prefix_${base}_males_imputed.dose.vcf.gz | bgzip -c > $dir/postimputation_files/$prefix/$prefix_${base}_males_imputed_bgzip.vcf.gz && tabix $dir/postimputation_files/$prefix/$prefix_${base}_males_imputed_bgzip.vcf.gz
    # Store header in a text file to modify it
    bcftools view -h $dir/postimputation_files/$prefix/$prefix_${base}_males_imputed_bgzip.vcf.gz > $dir/postimputation_files/hdrmales.txt
    ...
    bcftools reheader -h $dir/postimputation_files/hdrmales.txt --output $dir/postimputation_files/$prefix/$prefix_${base}_males_hdr_imputed_bgzip.vcf.gz $dir/postimputation_files/$prefix/$prefix_${base}_males_imputed_bgzip.vcf.gz
    tabix -p vcf $dir/postimputation_files/$prefix/$prefix_${base}_males_hdr_imputed_bgzip.vcf.gz
    # Compress the female imputed file with bgzip to avoid problems using bcftools afterwards
    zcat $dir/pimputed_files/$prefix/$prefix_${base}_females_imputed.dose.vcf.gz | bgzip -c > $dir/postimputation_files/$prefix/$prefix_${base}_females_imputed_bgzip.vcf.gz && tabix $dir/postimputation_files/$prefix/$prefix_${base}_females_imputed_bgzip.vcf.gz
    # Store header in a text file to modify it
    bcftools view -h $dir/postimputation_files/$prefix/$prefix_${base}_females_imputed_bgzip.vcf.gz > $dir/postimputation_files/hdrfemales.txt
    # Add lines to the header conatining information about ID=GENOTYPED (avoid problems in the next steps)
    sed -i '/^##FORMAT=<ID=GT/i ##FILTER=<ID=GENOTYPED,Description="Marker was genotyped AND imputed">\n##FILTER=<ID=GENOTYPED_ONLY,Description="Marker was genotyped but NOT imputed">' $dir/postimputation_files/hdrfemales.txt
    # Create a temporal file with the new header and the data from the imputed file
    bcftools reheader -h $dir/postimputation_files/hdrfemales.txt --output $dir/postimputation_files/$prefix/$prefix_${base}_females_hdr_imputed_bgzip.vcf.gz postimputation_files/$prefix/$prefix_${base}_females_imputed_bgzip.vcf.gz
    tabix -p vcf $dir/postimputation_files/$prefix/$prefix_${base}_females_hdr_imputed_bgzip.vcf.gz
    # Merge files containing the female and the male individuals 
    bcftools merge --force-samples --output-type z --output $dir/postimputation_files/TEMP1.vcf.gz $dir/postimputation_files/$prefix/$prefix_${base}_males_hdr_imputed_bgzip.vcf.gz $dir/postimputation_files/$prefix/$prefix_${base}_females_hdr_imputed_bgzip.vcf.gz
    ...
    then
        ...
        # Store header in a text file to modify it
        bcftools view -h $dir/postimputation_files/$prefix/$prefix_${base}_imputed_bgzip.vcf.gz > $dir/postimputation_files/hdr.txt
        # Add lines to the header conatining information about ID=GENOTYPED (avoid problems in the next steps) 
        sed -i '/^##FORMAT=<ID=GT/i ##FILTER=<ID=GENOTYPED,Description="Marker was genotyped AND imputed">\n##FILTER=<ID=GENOTYPED_ONLY,Description="Marker was genotyped but NOT imputed">' $dir/postimputation_files/hdr.txt
        # Create a temporal file with the new header and the data from the imputed file
        bcftools reheader -h $dir/postimputation_files/hdr.txt --output $dir/postimputation_files/TEMP1.vcf.gz $dir/postimputation_files/$prefix/$prefix_${base}_imputed_bgzip.vcf.gz
        tabix -p vcf $dir/postimputation_files/TEMP1.vcf.gz
    fi
  fi
  ...
  bcftools annotate --annotations $ref --columns ID --threads $cpus --output-type z --output $dir/${base}_imputed_files/$prefix/$prefix_${base}_imputed_final.vcf.gz $dir/postimputation_files/TEMP2.vcf.gz
  tabix -p vcf $dir/${base}_imputed_files/$prefix/$prefix_${base}_imputed_final.vcf.gz
  rm $dir/postimputation_files/TEMP* # Remove temporal files created during the process
  rm $dir/postimputation_files/hdr* # Remove files containing the headers
}
